...
export class CameraResolverBase {
  ...
  @graphql.ResolveField(() => [VideoSegment], { name: "videoSegments" })
  async findVideoSegments(
    @graphql.Parent() parent: Camera,
    @graphql.Args() args: VideoSegmentFindManyArgs
  ): Promise<VideoSegment[]> {
    const results = await this.service.findVideoSegments(parent.id, args);
  ...
  @graphql.Mutation(() => CameraDto)
  async CreateCamera(
    @graphql.Args()
    args: CameraDto
  ): Promise<CameraDto> {
    return this.service.CreateCamera(args);
  }

  @graphql.Query(() => [CameraDto])
  async GetCameras(
    @graphql.Args("args")
    args: string
  ): Promise<CameraDto[]> {
    return this.service.GetCameras(args);
  }
}
